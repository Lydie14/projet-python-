...
help_button = Button(420, 225, help_img, 1)     # declaration du carré 
historique_img = font.render('HISTORIQUE',True,col1) #  taille de HISTORIQUE 
...
while True:
    ...
    if commencer_button.draw(screen):# une fois cliqué sur le boutton , le jeu va se lancer 
        return main()
    ...
    if historique_button.draw(screen):
        print(score1,score2)
    if exit_button.draw(screen): # bloc d'instruction pour quitter 
        pygame.quit()
        sys.exit()
